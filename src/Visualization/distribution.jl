...
title_hist = Observable("Histogram of HR")
title_dist = Observable("Distribution of HR")

# Create the plots
fig = Figure(size=(1920, 1080));
ax_rr = Axis(fig[1, 1:5], title=title_rr, ylabel="Heart Rate (BPM)");
ax_hist = Axis(fig[1, 6:8], title=title_hist, ylabel="Heart Rate (BPM)");
ax_dist = Axis(fig[1, 9:10], title=title_dist, ylabel="Heart Rate (BPM)");

# hidespines!(ax_rm)
# hidedecorations!(ax_rm)

# Plot the initial data
lines!(ax_rr, t_rr, bpm, color=:teal);
hlines!(ax_rr, avg_bpm, color=:coral)

# Plot the histogram of the variable
hist!(ax_hist, bpm, color=:deeppink, bins=length(range(200,300, step=8)), direction=:x)
# hlines(ax_hist, [std(bpm[]), -1 * std(bpm[])], color=:red, linestyle=:dash)

# Plot the distribution of the variable
density!(ax_dist, bpm, color=:blueviolet, direction=:y)
...
while true
    ...
    t[] = t[]
    t_rr[] = t_rr[]
    println("$i at t: $(t[][i]) ($timestamp)/$(t_rr[][end]) : $(sample[1])")
