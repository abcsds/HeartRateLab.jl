...
Every feature has a full "Pokédex" entry in the [HRV Variable Zoo](zoo/index.md): definition, equation, normative distribution, resource cost, seminal citation, and per-field application evidence (clinical, sports & peak-performance, contemplative practice), all referenced against the [HRV knowledge base](zoo/references.md).
...
The short-term DFA exponent α₁ is available as the `dfa1` representation
(`HeartRateLab.Features.function_registry["dfa1"](m)`), a building block rather
than a registry feature, so it does not appear in `extract_feature_set` output.
