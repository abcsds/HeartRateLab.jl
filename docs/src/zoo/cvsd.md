...
| **Equation** | `sdsd / mean(IBI)` |
| **Resource intensity** | ◍◍◌◌◌  low — _Level 1–2 (base statistics over successive differences). Warm (shared representation cached) is 4× cheaper._ (measured, see §Resources) |

## Definition

Coefficient of variation of successive differences. Formally: `sdsd / mean(IBI)`.

## What does *normal* look like?

Fitted normative prior: **Gamma(α = 2.929, θ = 0.0146)**  —  KS p = 6.3e-44, n = 61715.

![Normative distribution of cvsd](figs/cvsd.png)
...
|---|---|
| median | 0.03348 |
| IQR (25–75%) | 0.02369 – 0.04859 |
| 5–95% range | 0.01718 – 0.09653 |
| mean ± sd | 0.04277 ± 0.03589 |
| n windows | 61715 |
...
- Heart-rate-normalised short-term variability (RMSSD / mean NN).
...
|---|---|
| cold median wall-time | 0.01371 ms |
| warm median wall-time | 0.003096 ms |
| allocations (cold) | 9.3 KiB |

*Cold* = fresh memoization caches (builds every shared representation from scratch); *warm* = shared representations (`diff`, periodogram, Poincaré coords, DFA fluctuation) already cached, so only this feature is recomputed. The tier is derived from the cold cost; see `Level 1–2 (base statistics over successive differences). Warm (shared representation cached) is 4× cheaper.`

## Citation

CV of successive differences (RMSSD/mean), Task Force (1996).
