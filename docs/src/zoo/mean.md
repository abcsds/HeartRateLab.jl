...
Mean inter-beat interval. Formally: `mean(IBI)`.

## What does *normal* look like?

Fitted normative prior: **Normal(μ = 779.6, σ = 146.8)**  —  KS p = 1.4e-64, n = 61715.

![Normative distribution of mean](figs/mean.png)
...
|---|---|
| median | 763.5 |
| IQR (25–75%) | 668.9 – 881.7 |
| 5–95% range | 566.4 – 1037 |
| mean ± sd | 779.6 ± 146.8 |
| n windows | 61715 |
...
- Baseline heart period; denominator for normalised HRV indices.
...
!!! note
    **Meditation's effect on HR level is disputed** — a large meta-analysis reports a pooled decrease ([pascoe2017](@cite)), but a more recent physiological study found HR unchanged (Chi meditation) or significantly *increased* (Kundalini Yoga), explicitly arguing raw HR is a poor real-time meditation biofeedback signal ([natarajan2023](@cite)).

See the [effect-distribution meta-analysis](../usecases/effect-distributions.md) page for the harvested per-study effect sizes/p-values behind these domain summaries (`docs/zoo_gen/effect_stats.csv`).

## Resources

Resource-intensity rank **◍◌◌◌◌  very low** is **measured** — median wall-clock time + allocations over a 360-beat window on synthetic realistic RR (`docs/zoo_gen/bench_resources.jl`; full grid in `resource_bench.csv`).

| metric (360-beat window) | value |
|---|---|
| cold median wall-time | 0.002846 ms |
| warm median wall-time | 0.003116 ms |
...
Basic descriptive statistic of the Task Force (1996) time-domain panel; no earlier single origin.
