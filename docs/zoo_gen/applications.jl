...
const FAMILY_APPLICATIONS = Dict{String,NamedTuple}(
    ...
    "global_variability" => (
        ...
        meditation = DomainApplication(
            ...
            "evidence for resting and vagally-mediated HRV (including SDNN) inconclusive or null, with " *
...
const VARIABLE_NOTES = Dict{String,Vector{String}}(
    "sd1" => [
        "**SD1 and RMSSD are the same statistic** (SD1 = RMSSD/sqrt(2)), " *
    ...
    "rmssd" => [
        "RMSSD and SD1 are the same statistic (SD1 = RMSSD/sqrt(2)); see the [`sd1`](sd1.md) entry. Papers that report RMSSD and SD1 as " *
        "separate, corroborating findings are re-describing one statistic ([ciccone2017](@cite)).",
        "**Meditation's effect on RMSSD/HR is disputed.** A meta-analysis and several individual " *
        "RCTs disagree on both the sign and the significance of the RMSSD response " *
        "(see the Contemplative practice section above).",
    ],
    "sd2_sd1" => [
