#!/usr/bin/env julia
# Clustermap in ONE coordinate system so the dendrogram rows line up exactly with the
# heatmap rows. Left (x<0) = dendrogram; right (x=1..n) = |corr| heatmap; same y-axis.
ENV["GKSwstype"]="100"
...
DSPAN=0.92*n; DSCALE=DSPAN/hmax; DW=DSPAN       # wide dendrogram band so the figure fills the card width

default(fontfamily="sans-serif",grid=false)
Cord=abs.(C)[ord,ord]
# Fill the poster card width: a prominent dendrogram band + heatmap. Cells carry
# only a mild aspect (~1.3:1), not the extreme stretch of a full-bleed square heatmap.
p=plot(size=(2720,1090),dpi=170,legend=false,framestyle=:none,
    xlims=(-DW-0.8, n+0.8), ylims=(-0.2, n+2.0),
    top_margin=4Plots.mm,bottom_margin=24Plots.mm,left_margin=6Plots.mm,right_margin=6Plots.mm)
# heatmap in x=1..n (same y-axis as dendrogram)
heatmap!(p, 1:n, 1:n, Cord; c=cgrad(:Blues), clims=(0,1), colorbar=true, colorbar_title="|r|")
# dendrogram: leaves at x=0, merges extend to NEGATIVE x (leftward), scaled to be visible
function draw!(id)
...
# column labels (rotated) UNDER the heatmap — rows are the same order
for (j,f) in enumerate(ord)
    annotate!(p, j, 0.10, text(get(LABc,good[f],good[f]), 12, DOM[featdom[good[f]]], :right, rotation=60))
end
# dendrogram scale ticks (negative x, in real 1−|r| units)
...
title!(p,"Feature redundancy — clustered |correlation|, healthy reference")
savefig(p,joinpath(ROOT,"docs/poster/figs/feature_clustermap.png"))
println("wrote feature_clustermap.png  (single-axis, aligned)")
